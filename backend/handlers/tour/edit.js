...
export async function edit({ send, error, db, data, user, files }) {
    ...
    let { public_id, title, location, price, description, from_datetime, to_datetime, participents_limit } = data;

    if(!user.is_admin)
        return error(FORBIDDEN)

    let { result: prev_tour } = await QueryExecutor('tour', db)
            .select('image')
            .where('public_id = ?', public_id)
            .runGetFirst()

    if(prev_tour == undefined)
    ...
    let tour = {
        ...
        from_datetime, to_datetime, participents_limit
    };

    if(tour.image){
        await fs.unlink(join(UPLOADS_DIRECTORY, prev_tour.image))
    }
